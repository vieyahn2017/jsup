...
def calHaohaoWuge(verbose=False):
    """
    方法二：
    三才五格 最适合的计算
...
def getSancaiWugeSelection(best_combination):
    ...
    print('三才五格待测试名字列表：', len(name_set))
    index = 0; results = []
    for x in name_set:
        index = index + 1
        results.append(x)
...
def mainBestSancaiwuge_yang():
    """
    直接写死，杨的五格
    这边的综合计算的结果，数量6, 组合
    [(3, 15, '火土金', '大吉'), (12, 6, '火土金', '大吉'), (12, 12, '火土火', '大吉'),
    (18, 6, ' 火木火', '大吉'), (18, 14, '火木木', '大吉'), (18, 17, '火木土', '大吉')]
    结合我自己的：
    本来只打算以13-18-14来取的，字太少。 13-18-17舍弃，因为字数原因。 13-3-15也舍弃，因为字的笔画跟别的没有重复，工作量不值
    """
    best_list = [(18, 14, '火木木', '大吉'), (18, 6, ' 火木火', '大吉'), (12, 6, '火土金', '大吉'), (12, 12, '火土火', '大吉')]
    sancaiwuge_sel = getSancaiWugeSelection(best_list)
    # print(sancaiwuge_sel)
