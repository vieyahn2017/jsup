...
calendar.StemsBranch= StemsBranch;
...
/* datetime: '2023-07-09 12:44:12'  其实里面那个正则替换没必要 '2023/07/09 12:44:12' 这两者都可以作为 new Date()的参数 */
function formatTime2(datetime, format) {
  return formatTime(new Date(datetime.replace(/-/g, '/')), format);
}

/** 时间戳转日期 1688901705328 > {year: 2023, month: 7, day: 9, hour: 19, minute: 21, …} */
function getLocalTimeObj (ns) {
  // needTime是整数，否则要parseInt转换
  var time = new Date(parseInt(ns) * 1); //根据情况*1000
  var y = time.getFullYear();
  var m = time.getMonth() + 1;
  ...
  var mm = time.getMinutes();
  var s = time.getSeconds();
  return {"year":y, "month":m, "day":d, "hour":h, "minute":mm, "second":s};
}

// from baby-shorthand-prod-master\miniprogram\utils\util.js
calendar.getLocalTime = getLocalTime;
calendar.formatTime = formatTime;
calendar.formatTime2 = formatTime2;
calendar.getLocalTimeObj = getLocalTimeObj;
