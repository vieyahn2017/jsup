...
cnchar.use(trad, radical, order, info);
...
let transRadicalStrokeIn5 = function (char) {
    // let trad = cnchar.convert.convertSimpleToTrad(char);
    let trad = char.convertSimpleToTrad();
    let stroke = cnchar.stroke(char);
    let stroke5 = cnchar.stroke(trad);
    let radicalRes = cnchar.radical(trad)[0]; // 到底用自身的部首，还是trad的部首。  // 已测到：茧字 例外了
    ...
    let infoRes = cnchar.info(char)[0];
    ...
    if (radical == '氵') {
        ...
        if (cnchar.stroke(trad, "order")[0][0] != "w") { // 右"阝""卩"算七画，如即、邓、邝等;
            radicalCount5 = 7;
            stroke5 = stroke5 + (radicalCount5 - radicalCount);
            // stroke5 = stroke + 5;
        } else {  // 左"阝"算八画，阳、阴、陈、陆等;
            radicalCount5 = 8;
            stroke5 = stroke5 + (radicalCount5 - radicalCount);
            // stroke5 = stroke + 6;
        }
    } else if (radical == '卩') { // 右"卩"算七画，如即、邓、邝等;
        radicalCount5 = 7;
        stroke5 = stroke5 + (radicalCount5 - radicalCount);
        // stroke5 = stroke + 5;
...
// {"char":"杨","trad":"楊","stroke":7,"stroke5":13,"radical":"木","radicalCount":4,"radicalCount5":4,"struct":"左右结构","fiveElement":"木"}
let transRadicalStroke5ToStr = function (char) {
	let item = transRadicalStrokeIn5(char);
	// let CNCHAR_FORMAT = "{0}({1})/{2}({3}) - radical:{4}({5}/{6}) - {7} - 五行:{8}";
	let CNCHAR_FORMAT = `${item.char}(${item.stroke})/${item.trad}(${item.stroke5}) - 五行:${item.fiveElement} - 偏旁:${item.radical}(${item.radicalCount}/${item.radicalCount5})`; //  - ${item.struct}
	return CNCHAR_FORMAT;
}

// https://blog.csdn.net/qq_34917408/article/details/128187549
...
function getShengxiaoLikeObj (shengxiao) {
    let sxpp = ShengXiaoMatch[shengxiao];
    return {"good": sxpp["good"].join(""), "bad": sxpp["bad"].join("")};
}
...
// addtions包含num, dict, filter等参数
// addtions = {"num": 5, "shengxiao": "兔", "dict_type": 1}
...
function getRandomItems(arr, count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const item = arr.splice(randomIndex, 1)[0]; /* 使用splice()方法从原数组中删除随机元素，以避免重复选择 */
        result.push(item);
    }
    return result;
}
...
let make5GCharDictBySX = function (str, shengxiao) {
    ...
    for(let i=0, leni=chars.length; i < leni; i++) {
        ...
        if (!checkShengxiao(item5g, shengxiao)) {
            ...
            // let sex = guessSex(char, item5g.radical);
            item5g.sex = "";
            arr.push(item5g);
            appends.push(char);
        }
...
let cncharGetWuXing = function (char) {
	return cnchar.info(char)[0].fiveElement;
}

// checkXiyongOfS("木火金", "金") == true
// checkXiyongOfS("木火金", "金,水") == false
// checkXiyongOfS("木水金", "金,水") == true
...
// addtions = {"num": 5, "shengxiao": "兔", "dict_type": 1}
// make5GName2("杨",12,23, {})
// make5GName2("杨",12,23, {"num": 10, "shengxiao": "兔", "xiyong": "金,水"})
// 五行喜用，一个方式是目前我的实现：过滤掉不符合的； 还有一种方式是像GoodGoodName那样生成五行的dict
let make5GName2 = function (xing, b, c, addtions) {
    let num = addtions.num || 5;
    let shengxiao = addtions.shengxiao || "兔";
    let dict_type = addtions.dict_type != undefined ? addtions.dict_type : 1;
	let dict_str = addtions.dict_str;
	let xiyong = addtions.xiyong || "";
    let use_dict = {"dict": {}, "txt": ""};
	if (shengxiao == "兔兔兔") {
	    // 之前想的，当年生肖预设字库。 目前暂时关闭
		if (dict_type == 3) {
			let dd = make5GCharDictBySX("铭韶瑜蝉织璨双环翡逢绵蜜鸣馥菲语侨笺菀筝嘉萌荣蕊谨归颜简荞蕙芸连通瑄对畅宁焘适瞻鲤丰礼维嫣璧", shengxiao);
			use_dict = dd.dict;
			console.log(333);
		}
	} else {
    ...
    let result = [];
    if (!use_dict[b]) return result;
    if (!use_dict[c]) return result;
...
let getXiYongTotal = function () {
	...
	// 定义3、4、5月为春天，6、7、8月为夏天，9、10、11月为秋天，12、1、2月为冬天。
	if (month >= 3 && month < 6) { // 春天出生的，多见甲乙寅卯木，壬癸亥子水，所以以土金为喜神、用神;
		return "土"; // "土,金"
	} else if (month >= 6 && month < 9) { // 夏天出生的人，八字里面没有壬癸亥子水和庚辛申酉金，所以以金水为喜用神;
		return "金"; // "金,水"
	} else if (month >= 9 && month < 12) { // 秋天出生的人，多见庚辛申酉金和戊己辰戌丑未土，所以以水木为喜神、用神。
		return "水"; // "水,木"
	} else { // 冬天出生的人，因为八字中没有丙丁巳午火，所以以火为喜用神;
		return "火";
	}
	return "";
}
...
const CYXS = [{"f": "杨","n": 13}, {"f": "文","n": 4}, {"f": "李","n": 7}, {"f": "刘","n": 15},
...
function getFamilyNumber (familyname) {
	...
	// 不需要用 ?. 判空  == 参考 return res?.length? res.join("") : "";
...
const fn_num_5G_dict = {
    ...
    "13": ["13-18-14", "13-8-16", "13-12-6", "13-12-12", "13-3-15", "13-18-17", "13-18-6", "13-12-4"],
...
// 蕊宁  // dictB = transRadicalStrokeIn5("蕊"); dictC = transRadicalStrokeIn5("福");
let checkNameMatch5GPrio = function (charB, charC, xing_num) {
    let dictB = transRadicalStrokeIn5(charB);
    let dictC = transRadicalStrokeIn5(charC);
    let xing_5g_prio = [];
    if (xing_num) {
        // ss.map((x) => {arr=x.split("-"); return arr[1]+'-'+arr[2]}).sort();
        xing_5g_prio = fn_num_5G_dict[xing_num].map((x) => {arr=x.split("-"); return [arr[1], arr[2]]}); //.sort();
    } else {
        xing_5g_prio = fn_num_5G_dict["13"].map((x) => {arr=x.split("-"); return [arr[1], arr[2]]}); // .sort();
    }
...
// findNamesInLine("杨环维杨秋维杨蕊福杨蕊宁杨蕊蕊新消息先宁", "杨", 13)
// ['杨环维', '杨秋维', '杨蕊福', '杨蕊宁']
